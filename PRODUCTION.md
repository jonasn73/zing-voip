...
| `ZING_AI_LAST_RESORT_CONNECT_HIT` | Optional. Default **`5`**. On that **`/incoming`** POST count for the call, Zing returns **`<Connect><AIAssistant>`** on `/incoming` after redirect retries (Telnyx often loops `/incoming`↔`/ai-bridge` without attaching AI). If loops continue, the next hit **hangs up** with a clear message. Set **`0`** or **`false`** to disable (shorter silent cap only). |
