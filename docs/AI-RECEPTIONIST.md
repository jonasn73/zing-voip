...
8. **Ring-first path (`ZING_AI_RING_OWNER_FIRST`):** `/fallback` uses **`/fallback/u/{userId}/n/{digits}/{mode}`**, path **`mode=owner-ai`**, **`zingFbMode=`** fallback, and safe merge rules. For **`owner-ai`**, the caller is **always** offered Voice AI when your cell leg ends (decline, short answer + hang-up, or long bridged call). **Early hang-up** (drop caller, no AI) still applies only on **`recv-ai`** after a **2+ minute** bridged receptionist leg (`DialBridgedTo` 10+ digits, duration ≥ 2 minutes).
