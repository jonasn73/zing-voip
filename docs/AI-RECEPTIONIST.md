...
5. **Default inbound (AI + no receptionist):** when **AI fallback** is on, the caller first hears a short **“Thanks for calling…”** line **before** `<Dial>` (so something always plays even if later steps misbehave). Your **cell rings next**, then `/fallback` returns **Say + Pause + Redirect** to **`/api/voice/telnyx/ai-bridge/u/{userId}`**, which returns the pure **`<Connect><AIAssistant>`** (see item 4).
