...
8. **“Pre-line plays, then voicemail instead of AI”:** Dial `action` webhooks often send **`To` = your cell**, not the public DID. Zing must **not** treat that as the business line, or routing can fall back to the **default** row (e.g. voicemail) instead of the **per-number AI** row. The handler uses `bn=` / `OriginalCalledNumber` / primary-DID recovery and **`leg=owner-first`** on the owner ring so AI fallback still applies.
