...
6. **Ring your phone before AI:** Dashboard → **Fallback Settings** → **AI receptionist** → turn on **Ring my phone first** (no active receptionist). Your cell rings first; if you don’t answer, Voice AI runs — good for testing the full path. Still optional: **`ZING_AI_RING_OWNER_FIRST=true`** in Vercel forces the same for everyone. Requires Neon migration **`015-routing-ai-ring-owner-first.sql`** for the in-app toggle.
