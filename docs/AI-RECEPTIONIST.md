...
8. **“Pre-line plays, then voicemail instead of AI”:** Dial `action` webhooks often send **`To` = your cell**, not the public DID. Zing must **not** treat that as the business line, or routing can fall back to the **default** row (e.g. voicemail) instead of the **per-number AI** row. The handler uses **`/fallback/u/{userId}/n/{digits}`** (DID in the **path** so it is not dropped), `bn=` / `OriginalCalledNumber`, primary-DID recovery, **`leg=owner-first`**, and **`fb=ai`** (set by `/incoming` when that line is AI) so merge mistakes still hand off to Voice AI on the **owner** leg.
