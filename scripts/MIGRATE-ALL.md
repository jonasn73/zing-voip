...
| 53 | `053-invitations.sql` | **`invitations`** table (`target`, `type` EMAIL/SMS, `token`, `status`, `expires_at`) backing the admin "Invite receptionist" modal → `/register?token=`. **Optional** — the app auto-creates it at runtime via `CREATE TABLE IF NOT EXISTS`; run only if you prefer to pre-create it. |
